...
function prepararModuloTemporal() {
  const carpetaTemporal = fs.mkdtempSync(path.join(os.tmpdir(), "ot0271-hidroflow-"));
...
salidaMarkdown.push("");
salidaMarkdown.push("- La función auxiliar quedó delegada al helper validado.");
...
salidaMarkdown.push("");
salidaMarkdown.push(
  resumen.requiereCorreccionSalidaReal
    ? "El acople auxiliar queda validado, pero la salida real requiere una OT posterior de corrección controlada."
    : "El acople queda validado tanto en función auxiliar como en salida real."
);
salidaMarkdown.push("");
...
salidaMarkdown.push(
  resumen.requiereCorreccionSalidaReal
    ...
    : "`OT-0272 — Decisión estabilización bloque Tiempo de concentración y roles Tc del expediente`"
);

const rutaSalida = "00_ADMIN/bitacora/OT-0271/OT-0271B_validacion_acople_helper_tiempo_concentracion_roles_tc_expediente.md";

fs.mkdirSync("00_ADMIN/bitacora/OT-0271", { recursive: true });
...
console.log("VALIDACION_OT_0271_ACOPLE_TC_ROLES_COMPLETADA");
