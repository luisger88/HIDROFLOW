...
function prepararModuloTemporal(fuenteExpediente) {
  const carpetaTemporal = fs.mkdtempSync(path.join(os.tmpdir(), "ot0289-hidroflow-"));
...
const patronInlineAntiguoVolumen =
  /"## 4\. Volumen de referencia",\s*`Lluvia efectiva total:\s*\$\{[\s\S]*?`Volumen esperado:\s*\$\{[\s\S]*?"Fórmula: Pe\(mm\) × Área\(km²\) × 1000\.",/;
...
const controles = [
  ...
  },
  {
    ...
    aprobado:
      ...
      tramoConstructorAntesSeccion5.includes("peTotalMmBloqueVolumenReferenciaDocumental") &&
      tramoConstructorAntesSeccion5.includes("volumenEsperadoM3BloqueVolumenReferenciaDocumental")
  },
  {
    ...
    aprobado:
      !patronInlineAntiguoVolumen.test(tramoConstructorAntesSeccion5)
  },
  {
    id: "entradas_documentales_explicitas_presentes",
    descripcion: "Constructor recibe entradas documentales explícitas para Volumen",
    aprobado:
      fuenteExpediente.includes("peTotalMm: peTotalMmEntradaDocumental = null") &&
      fuenteExpediente.includes("volumenEsperadoM3: volumenEsperadoM3EntradaDocumental = null")
  },
  {
    id: "variables_documentales_respaldo_presentes",
    descripcion: "Variables documentales de respaldo para Volumen presentes",
    aprobado:
      fuenteExpediente.includes("peTotalMmBloqueVolumenReferenciaDocumental") &&
      fuenteExpediente.includes("volumenEsperadoM3BloqueVolumenReferenciaDocumental")
  },
  {
...
const salidaMarkdown = [
  "# OT-0289B — Revalidación acople helper bloque Volumen de referencia del expediente",
  "",
  "## Resumen",
  "",
  "```json",
...
salidaMarkdown.push("");
salidaMarkdown.push("- El acople del helper Volumen de referencia fue revalidado estructuralmente.");
salidaMarkdown.push("- La función auxiliar delega al helper validado.");
salidaMarkdown.push("- La salida real usa la función auxiliar con variables documentales de respaldo.");
salidaMarkdown.push("- El bloque inline antiguo no permanece en el constructor principal.");
salidaMarkdown.push("- La salida real conserva las líneas esperadas con valores documentales.");
...
salidaMarkdown.push(
  resumen.acopleValidado
    ? "El acople mínimo del helper `construirBloqueVolumenReferenciaExpediente` queda revalidado."
...
salidaMarkdown.push(
  resumen.acopleValidado
    ...
    : "`OT-0290 — Corrección adicional acople helper bloque Volumen de referencia del expediente`"
);

const rutaSalida = "00_ADMIN/bitacora/OT-0289/OT-0289B_revalidacion_acople_helper_bloque_volumen_referencia_expediente.md";

fs.mkdirSync("00_ADMIN/bitacora/OT-0289", { recursive: true });
...
console.log("VALIDACION_OT_0289_ACOPLE_HELPER_VOLUMEN_REFERENCIA_COMPLETADA");
