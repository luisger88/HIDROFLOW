...
function prepararModuloTemporal() {
  const carpetaTemporal = fs.mkdtempSync(path.join(os.tmpdir(), "ot0258-hidroflow-"));
  ...
  let fuenteTemporal = fuenteExpediente
    .replace(
...
const inicioTextoInlineParametros =
  inicioConstructor >= 0
    ? fuenteExpediente.indexOf('"## 2. Parámetros hidrológicos base"', inicioConstructor)
    : -1;
const inicioTextoSeccion3 =
  inicioTextoInlineParametros >= 0
    ? fuenteExpediente.indexOf('"## 3. Tiempo de concentración y roles Tc"', inicioTextoInlineParametros)
    : -1;

const bloqueInlineConstructor =
  inicioTextoInlineParametros >= 0 && inicioTextoSeccion3 >= 0
    ? fuenteExpediente.slice(inicioTextoInlineParametros, inicioTextoSeccion3)
    : "";
...
const controles = [
  ...
  },
  {
    ...
    bloqueInlineConstructorDetectado: bloqueInlineConstructor.length > 0,
    aprobado: bloqueInlineConstructor.length === 0
  },
  {
...
salidaMarkdown.push("");
salidaMarkdown.push("- La validación revisó fuente, import, función auxiliar, salida directa y salida real del constructor principal.");
salidaMarkdown.push("- La función auxiliar se evalúa con valores objeto para detectar si la normalización proviene del helper.");
salidaMarkdown.push("- Si la salida real contiene `[object Object]`, significa que el constructor principal conserva lógica inline no delegada.");
salidaMarkdown.push("- No se modificó código funcional durante esta validación.");
...
salidaMarkdown.push(
  resumen.acopleSalidaRealValidado
    ? "El acople del helper de Parámetros hidrológicos base queda validado también en salida real."
...
salidaMarkdown.push(
  resumen.acopleSalidaRealValidado
    ? "`OT-0259 — Decisión estabilización bloque Parámetros hidrológicos base del expediente`"
    : "`OT-0259 — Corrección acople helper Parámetros hidrológicos base del expediente`"
);

const rutaSalida = "00_ADMIN/bitacora/OT-0258/OT-0258B_validacion_acople_helper_parametros_hidrologicos_base_expediente.md";

fs.mkdirSync("00_ADMIN/bitacora/OT-0258", { recursive: true });
...
console.log("VALIDACION_OT_0258_ACOPLE_PARAMETROS_BASE_COMPLETADA");
