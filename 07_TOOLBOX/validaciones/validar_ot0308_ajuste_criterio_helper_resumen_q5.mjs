...
const ocurrenciasHelperNuevoEnConstructor = contarOcurrencias(
  fuenteConstructor,
  "construirBloqueResumenQ5AuditadoExpediente"
);

const ocurrenciasAuxiliarPreexistenteEnConstructor = contarOcurrencias(
  fuenteConstructor,
...
const observaciones = [
  {
    ...
    descripcion:
      "El contrato documental mostró la salida mínima con punto final en el estado; la implementación emite el estado sin punto final. Se mantiene como observación no bloqueante.",
  ...
  },
  {
    ...
    descripcion:
      "OT-0308 ajusta el criterio de aislamiento para comprobar solo que el constructor no importe ni use el helper nuevo construirBloqueResumenQ5AuditadoExpediente. La presencia preexistente de construirLineasResumenQ5AuditadoExpediente no se considera acople del helper nuevo.",
    ocurrenciasHelperNuevoEnConstructor,
    ocurrenciasAuxiliarPreexistenteEnConstructor,
    bloqueante: false
  }
];
...
const controles = [
  ...
  },
  {
    ...
    descripcion:
      "Constructor principal no importa ni usa todavía el helper nuevo construirBloqueResumenQ5AuditadoExpediente",
    ocurrenciasHelperNuevoEnConstructor,
    ocurrenciasAuxiliarPreexistenteEnConstructor,
    aprobado: ocurrenciasHelperNuevoEnConstructor === 0
  },
  {
...
salidaMarkdown.push("");
salidaMarkdown.push("- Se ajustó el criterio de validación aislada para distinguir el helper nuevo de funciones auxiliares preexistentes del constructor.");
salidaMarkdown.push("- El constructor no importa ni usa `construirBloqueResumenQ5AuditadoExpediente`.");
salidaMarkdown.push("- La presencia preexistente de `construirLineasResumenQ5AuditadoExpediente` no se interpreta como acople del helper nuevo.");
salidaMarkdown.push("- El helper `construirBloqueResumenQ5AuditadoExpediente` queda validado en aislamiento si todos los controles aprueban.");
...
salidaMarkdown.push(
  resumen.helperValidadoAislado
    ...
    : "El helper requiere corrección adicional antes de avanzar."
);
salidaMarkdown.push("");
...
salidaMarkdown.push(
  resumen.helperValidadoAislado
    ...
    : "`OT-0309 — Corrección helper bloque Resumen Q-5 auditado del expediente`"
);

const rutaSalida = "00_ADMIN/bitacora/OT-0308/OT-0308B_ajuste_criterio_validacion_aislada_helper_resumen_q5.md";

fs.mkdirSync("00_ADMIN/bitacora/OT-0308", { recursive: true });
fs.writeFileSync(rutaSalida, salidaMarkdown.join("\n"), "utf8");

console.log("VALIDACION_OT_0308_AJUSTE_CRITERIO_HELPER_RESUMEN_Q5_COMPLETADA");
