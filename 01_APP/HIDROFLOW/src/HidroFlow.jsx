...
function OutletMiniMap({ lat, lon, alt, idf }) {
  const W = 480;
  const H = 165;

  // ------------------------------------------------------------
  // 1. Estaciones EPM disponibles para contexto geográfico
  // ------------------------------------------------------------
  const ests = Object.entries(ESTACIONES_EPM).map(([n, e]) => ({
    n,
    lat: e.lat,
    lon: e.lon,
    alt: e.alt
  ...
  // Tres estaciones más cercanas al punto de salida.
  // Estas estaciones son referencia geográfica, no IDF adoptada.
  const cercanas = [...ests]
    .sort(
      (a, b) =>
        distKm(lat, lon, a.lat, a.lon) -
        distKm(lat, lon, b.lat, b.lon)
    )
    .slice(0, 3);

  // ------------------------------------------------------------
  // 2. Estación IDF adoptada desde cuencasCatalogo.js
  // ------------------------------------------------------------
  const idfAdoptadaNombre =
    idf?.estacion_nombre ||
    idf?.estacion_id?.replaceAll("_", " ") ||
    "SAN CRISTOBAL";

  const idfAdoptadaLabel =
    idf?.estacion_label ||
    idf?.estacion_nombre ||
    "San Cristóbal";
  ...
  const estacionIDF =
    ESTACIONES_EPM[idfAdoptadaNombre] ||
    ESTACIONES_EPM["SAN CRISTOBAL"];

  const puntoIDF = estacionIDF
    ? {
        n: idfAdoptadaLabel,
        lat: estacionIDF.lat,
        lon: estacionIDF.lon,
        alt: estacionIDF.alt,
        dist: distKm(lat, lon, estacionIDF.lat, estacionIDF.lon)
      }
    : null;
  ...
  const lats = puntosMapa.map((p) => p.lat);
  const lons = puntosMapa.map((p) => p.lon);
  ...
  const padLat = Math.max((maxLat - minLat) * 0.35, 0.035);
  const padLon = Math.max((maxLon - minLon) * 0.35, 0.035);
  ...
  const lineaIDF = puntoIDF
    ? {
        x1: xMap(lon),
        y1: yMap(lat),
        x2: xMap(puntoIDF.lon),
        y2: yMap(puntoIDF.lat)
      }
    : null;
  ...
  return (
    <div style={{ marginTop: 12 }}>
      {/* Encabezado técnico del mini-mapa */}
      <div
        style={{
          display: "flex",
      ...
      >
        ...
        >
          ...
          >
            ◆ IDF adoptada: {idfAdoptadaLabel} · Método {idfMetodo}
          </span>
        ...
        >
          ...
          >
            Referencia geográfica — estaciones cercanas:
          </span>
          ...
          {cercanas.map((e) => (
            <span
              ...
              style={{
                ...
                border:
                  e.n === "CUCARACHO"
                  ? "1px solid rgba(255,180,80,0.42)"
                  : `1px solid ${C.accent2}28`,
                color:
                  e.n === "CUCARACHO"
                    ? "#ffb450"
                    : C.accent2,
      ...
      <div
        style={{
          ...
          borderRadius: 8,
          background:
            "linear-gradient(180deg, rgba(5,14,28,0.98), rgba(3,10,20,0.98))",
          position: "relative",
          overflow: "hidden",
          boxShadow: "inset 0 0 0 1px rgba(0,210,255,0.05)"
        }}
      >
        {/* Grilla horizontal */}
        {[0, 1, 2, 3, 4].map((i) => (
          <div
            ...
            style={{
              ...
              right: 0,
              top: `${15 + i * 18}%`,
              borderTop: "1px solid rgba(90,130,180,0.10)"
            }}
          />
        ))}
        ...
        {[0, 1, 2, 3, 4].map((i) => (
          <div
            ...
            style={{
              ...
              bottom: 0,
              left: `${12 + i * 20}%`,
              borderLeft: "1px solid rgba(90,130,180,0.07)"
            }}
          />
        ))}
...
<div
  style={{
    ...
    height: "84%",
    background:
      "linear-gradient(180deg, rgba(0,150,255,0.12), rgba(0,190,255,0.68), rgba(0,150,255,0.12))",
    borderRadius: 999,
    boxShadow: "0 0 16px rgba(0,180,255,0.38)",
    zIndex: 2
  }}
/>
...
<div
  style={{
    ...
    borderRadius: 14,
    background:
      "linear-gradient(90deg, rgba(0,230,184,0.055), rgba(0,230,184,0.018))",
    border: "1px dashed rgba(0,230,184,0.18)",
    boxShadow: "inset 0 0 18px rgba(0,230,184,0.035)",
    zIndex: 1
  }}
/>
...
>
  Occidente del Río Medellín · Subcuenca La Iguaná PC_80
    ...
    Se evita sugerir cruce físico del Río Medellín. */}
        ...
        {/* Estaciones cercanas: referencia geográfica */}
        {cercanas.map((e) => (
          <div
            key={`cercana-${e.n}`}
            title={`${e.n} · ${distKm(lat, lon, e.lat, e.lon).toFixed(1)} km`}
            style={{
              position: "absolute",
              left: xMap(e.lon),
              top: yMap(e.lat),
              ...
              background:
                e.n === "CUCARACHO"
                  ...
                  : "rgba(105, 145, 190, 0.60)",
              transform: "translate(-50%, -50%)",
              boxShadow:
                e.n === "CUCARACHO"
                  ? "0 0 13px rgba(255,180,80,0.70)"
                  : "0 0 7px rgba(105,145,190,0.38)",
        ...
        {/* Estación IDF adoptada: San Cristóbal */}
        {puntoIDF && (
          <>
            <div
              title={`${puntoIDF.n} · IDF adoptada · ${puntoIDF.dist.toFixed(
                1
              )} km`}
              style={{
                position: "absolute",
                ...
                boxShadow: "0 0 18px rgba(255, 209, 102, 0.85)",
                zIndex: 7
              }}
            />
        ...
        <>
          <div
            ...
            style={{
              ...
              transform: "translate(-50%, -50%)",
              boxShadow: "0 0 22px rgba(0, 230, 184, 0.90)",
              zIndex: 9
            }}
          />
        ...
        >
          <div style={{ color: "#00e6b8", fontWeight: 800 }}>
            ● Punto de salida activo
          </div>
          <div>Lat: {lat.toFixed(6)}</div>
          <div>Lon: {lon.toFixed(6)}</div>
          <div>Alt: {Number(alt || 0).toFixed(2)} msnm</div>
        </div>
      ...
      >
        ...
        <span>
          <span style={{ color: "#ffd166", fontWeight: 900 }}>◆</span>{" "}
          IDF adoptada
        </span>

        <span>
          <span style={{ color: "rgba(105,145,190,0.85)", fontWeight: 900 }}>
            ●
          </span>{" "}
          estaciones cercanas
        </span>
...
function ModHidrogramas({ params, est, name, onContextoComparador }) {
  ...
  const obtenerSerie = (h) => {
  if (Array.isArray(h?.serie)) return h.serie;
  if (Array.isArray(h?.hidrograma)) return h.hidrograma;
  if (Array.isArray(h?.data)) return h.data;
  if (Array.isArray(h?.puntos)) return h.puntos;
  if (Array.isArray(h?.qt)) return h.qt;
  if (Array.isArray(h?.Q_t)) return h.Q_t;
  if (Array.isArray(h?.valores)) return h.valores;
  ...
  if (Array.isArray(punto)) {
    return numeroValido(punto[1] ?? punto[0]?.q ?? punto[0]?.Q);
  }
...
const leerT = (punto, indice) => {
  if (Array.isArray(punto)) {
    return numeroValido(punto[0]) ?? indice * dtMin;
  }
  ...
  const calcularDesdeSerie = (h) => {
    const serie = obtenerSerie(h);
    ...
    serie.forEach((punto, indice) => {
      const q = leerQ(punto);
      const t = leerT(punto, indice);

      if (q !== null && (QpSerie === null || q > QpSerie)) {
        QpSerie = q;
        TpSerie = t;
      }

      if (indice > 0) {
        const q0 = leerQ(serie[indice - 1]);
        const q1 = q;
        const t0 = leerT(serie[indice - 1], indice - 1);
        const t1 = t;
    ...
    return {
      QpSerie,
      TpSerie,
      volumenSerie: volumenSerie > 0 ? volumenSerie : null,
      puntos: serie.length,
    };
  };

  const hidrogramasResumen = Array.isArray(hidros)
    ? hidros.map((h, i) => {
        const calculado = calcularDesdeSerie(h);

        const QpDirecto = numeroValido(
  h?.Qpico ??
    h?.Qp ??
    h?.qp ??
    h?.q_pico ??
  ...
  h?.tPico ??
    ...
    h?.tiempo_pico
        ...
        const volumenDirecto = numeroValido(
  h?.volTotal ??
    h?.volumen ??
    h?.V ??
    h?.vol ??
    h?.volume
        ...
        return {
          metodo: nombresHidrogramas[i] ?? `Método ${i + 1}`,
          Qp:
            QpDirecto && QpDirecto > 0
              ? QpDirecto
              : calculado.QpSerie,
          Tp:
            TpDirecto && TpDirecto > 0
              ? TpDirecto
              : calculado.TpSerie,
          volumen:
            volumenDirecto && volumenDirecto > 0
              ? volumenDirecto
              : calculado.volumenSerie,
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
      ...
      <div className="flex flex-wrap gap-4 p-3 mb-2 bg-slate-900 text-white rounded-md text-sm font-mono">
       ...
       <div className="text-slate-400">
         ...
         <div>• Comparador: pendiente · referencia especializada</div>
       </div>
...
// ------------------------------------------------------------
const [tabInterno, setTabInterno] = useState("params");

// Si viene del layout, usar ese.
